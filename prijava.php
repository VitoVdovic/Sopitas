...
<article>
    <div class='mb-3' id='naslov_forme' style="margin-left: 0px; width:150%;">
        <h1> Prijava </h1>
        <h2> Unesite podatke za prijavu: </h2>
    </div>
    <div class="container mt-5">
        <?php
        $korisnickoImeErr = $lozinkaErr = "";
        $korisnickoIme = "";
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $sql = "SELECT korisnickoIme, lozinka FROM korisnik WHERE korisnickoIme = ?";
            $stmt = $conn->prepare($sql);

            $stmt->bind_param("s", $korisnickoImeParam);

            $korisnickoImeParam = test_input($_POST["korisnickoIme"]);
            $lozinka = test_input($_POST["lozinka"]);

            if (empty($korisnickoImeParam)) {
            ...
            if (empty($korisnickoImeErr) && empty($lozinkaErr)) {
                $stmt->execute();
                ...
                if ($result->num_rows == 1) {
                    ...
                    if (password_verify($lozinka, $row["lozinka"])) {
                        $_SESSION['korisnickoIme'] = $korisnickoImeParam;
                        echo '<div class="alert alert-success" role="alert">
                                Uspješno ste prijavljeni kao ' . $korisnickoImeParam . '!
                              </div>';
                    } else {
                        $lozinkaErr = "Pogrešna lozinka";
                    }
                } else {
                    $korisnickoImeErr = "Korisnik ne postoji";
                }
            }
        ...
        ?>
        <form id='prijavaForma' name='prijavaForma' method='post' action='<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>'>
            <div class='mb-3'>
                <label for='korisnickoIme' class='form-label'>Korisničko ime:</label>
                <input type='text' class='form-control <?php if(!empty($korisnickoImeErr)) echo 'is-invalid'; ?>' id='korisnickoIme' name='korisnickoIme' value='<?php echo $korisnickoIme; ?>' required>
                <div class='invalid-feedback'><?php echo $korisnickoImeErr;?></div>
            </div>
            <div class='mb-3'>
                <label for='lozinka' class='form-label'>Lozinka:</label>
                <input type='password' class='form-control <?php if(!empty($lozinkaErr)) echo 'is-invalid'; ?>' id='lozinka' name='lozinka' required>
                <div class='invalid-feedback'><?php echo $lozinkaErr;?></div>
            </div>
            <button type='submit' class='btn btn-primary'>Prijavi se</button>
        </form>
        <div class="mt-3">
            <p>Novi korisnik? <a href="registracija.php">Registriraj se</a>.</p>
        </div>
    </div>
</article>
